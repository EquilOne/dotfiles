"""Auto-generated Moshi observer plugin for Hermes Agent."""
...
HELPER = "/home/chase/.local/bin/moshi-hook"
...
def _send(event_name, session_id="", **extra):
    ...
    # A single worker preserves Hermes hook order (SessionStart before the
    # first prompt, prompt before completion) while callbacks return quickly.
    # ThreadPoolExecutor drains submitted work during normal interpreter exit.
    try:
        _delivery.submit(_deliver, payload)
    except RuntimeError:
        # A finalization hook can run during interpreter shutdown after the
        # executor has stopped accepting work. Deliver that last event inline.
        _deliver(payload)
...
def on_pre_llm_call(session_id="", user_message="", model="", platform="", **kwargs):
    _send("UserPromptSubmit", _session(session_id, kwargs), prompt=user_message, model=model, platform=platform)

def on_post_llm_call(session_id="", assistant_response="", model="", platform="", **kwargs):
    _send("AgentEnd", _session(session_id, kwargs), last_assistant_message=assistant_response, model=model, platform=platform)

def on_session_end(session_id="", completed=False, interrupted=False, model="", platform="", **kwargs):
    if interrupted or not completed:
        _send("TurnInterrupted", _session(session_id, kwargs), interrupted=bool(interrupted), model=model, platform=platform)
...
def on_pre_approval_request(command="", description="", pattern_key="", pattern_keys=None, session_key="", surface="", **kwargs):
    if surface != "cli":
        return
...
def on_post_approval_response(command="", description="", pattern_key="", pattern_keys=None, session_key="", surface="", choice="", **kwargs):
    if surface != "cli":
        return
    key = _approval_key(session_key, command, pattern_key)
    with _pending_lock:
