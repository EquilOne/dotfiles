...
Skill Judge exists to solve a critical problem: **most Skills waste tokens on knowledge the model already has**.

The skill helps you evaluate whether a Skill actually adds value by measuring its "knowledge delta" - the gap between what the Skill provides and what the model already knows. A good Skill should be a compressed expert brain, not a tutorial.
...
- Token-wasting redundant content (things the model already knows)
...
1. **First Pass - Knowledge Delta Scan**: Read the SKILL.md and categorize each section as:
   ...
   - **[R] Redundant**: The model definitely knows this (should delete)
...
1. **The Tutorial**: Explains basics the model already knows
...
- Ask "Does the model already know this?" for every section
...
If yes, the Skill has genuine value. If no, it's compressing what the model already knows.
...
- **Tool vs Skill**: Tools define capability boundaries (what the model CAN do). Skills inject knowledge (what the model KNOWS how to do).
